...
@WebMvcTest(
        controllers = ReportDefinitionController.class,
...
class ReportControllerSecurityTest {
    ...
    @Test
    void listReportsUnauthorizedWithoutToken() throws Exception {
        mockMvc.perform(get("/api/v1/reports")).andExpect(status().isUnauthorized());
    }

    @Test
    @WithMockUser(authorities = "CLIENT_VIEW")
    void listReportsForbiddenWithoutPermission() throws Exception {
    ...
    void listReportsAllowedWithPermission() throws Exception {
        when(definitionService.list(isNull(), isNull(), any())).thenReturn(new PageResponse<>(List.of(), 0, 20, 0, 0));
        mockMvc.perform(get("/api/v1/reports"))
