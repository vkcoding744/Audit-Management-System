...
class InvoiceControllerSecurityTest {
    ...
    @Test
    void listInvoicesUnauthorizedWithoutToken() throws Exception {
        mockMvc.perform(get("/api/v1/invoices")).andExpect(status().isUnauthorized());
    }

    @Test
    @WithMockUser(authorities = "CLIENT_VIEW")
    void listInvoicesForbiddenWithoutPermission() throws Exception {
    ...
    void listInvoicesAllowedWithPermission() throws Exception {
        when(invoiceService.list(isNull(), isNull(), any())).thenReturn(new PageResponse<>(List.of(), 0, 20, 0, 0));
        mockMvc.perform(get("/api/v1/invoices"))
