...
class CertificateControllerSecurityTest {
    ...
    @Test
    void listCertificatesUnauthorizedWithoutToken() throws Exception {
        mockMvc.perform(get("/api/v1/certificates")).andExpect(status().isUnauthorized());
    }

    @Test
    @WithMockUser(authorities = "CLIENT_VIEW")
    void listCertificatesForbiddenWithoutPermission() throws Exception {
    ...
    @Test
    @WithMockUser(authorities = "CERTIFICATE_VIEW")
    void listCertificatesAllowedWithPermission() throws Exception {
        when(certificateService.list(isNull(), isNull(), any())).thenReturn(new PageResponse<>(List.of(), 0, 20, 0, 0));
        mockMvc.perform(get("/api/v1/certificates"))
