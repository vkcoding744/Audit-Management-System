...
@WebMvcTest(
        controllers = StandardController.class,
...
class StandardControllerSecurityTest {
    ...
    @Test
    void listStandardsUnauthorizedWithoutToken() throws Exception {
        mockMvc.perform(get("/api/v1/standards")).andExpect(status().isUnauthorized());
    }

    @Test
    @WithMockUser(authorities = "CLIENT_VIEW")
    void listStandardsForbiddenWithoutPermission() throws Exception {
    ...
    void listStandardsAllowedWithPermission() throws Exception {
        when(standardService.list(isNull(), isNull(), any())).thenReturn(new PageResponse<>(List.of(), 0, 20, 0, 0));
        mockMvc.perform(get("/api/v1/standards"))
