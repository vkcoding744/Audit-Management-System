...
class SiteControllerSecurityTest {
    ...
    @Test
    void listSitesUnauthorizedWithoutToken() throws Exception {
        mockMvc.perform(get("/api/v1/clients/c1/sites")).andExpect(status().isUnauthorized());
    }

    @Test
    @WithMockUser(authorities = "CLIENT_VIEW")
    void listSitesForbiddenWithoutPermission() throws Exception {
        mockMvc.perform(get("/api/v1/clients/c1/sites"))
                .andExpect(status().isForbidden())
                .andExpect(jsonPath("$.error.code").value("SYS_FORBIDDEN"));
    }

    @Test
    @WithMockUser(authorities = "SITE_VIEW")
    void listSitesAllowedWithPermission() throws Exception {
        when(siteService.listByClient("c1")).thenReturn(List.of());
        mockMvc.perform(get("/api/v1/clients/c1/sites"))
