...
class ClientControllerSecurityTest {
    ...
    @Test
    void listClientsUnauthorizedWithoutToken() throws Exception {
        mockMvc.perform(get("/api/v1/clients")).andExpect(status().isUnauthorized());
    }

    @Test
    @WithMockUser(authorities = "AUDIT_VIEW")
    void listClientsForbiddenWithoutPermission() throws Exception {
    ...
    void listClientsAllowedWithPermission() throws Exception {
        when(clientService.list(isNull(), isNull(), any())).thenReturn(new PageResponse<>(List.of(), 0, 20, 0, 0));
        mockMvc.perform(get("/api/v1/clients"))
