...
class LeadControllerSecurityTest {
    ...
    @Test
    void listLeadsUnauthorizedWithoutToken() throws Exception {
        mockMvc.perform(get("/api/v1/leads")).andExpect(status().isUnauthorized());
    }

    @Test
    @WithMockUser(authorities = "CLIENT_VIEW")
    void listLeadsForbiddenWithoutPermission() throws Exception {
    ...
    void listLeadsAllowedWithPermission() throws Exception {
        when(leadService.list(isNull(), any())).thenReturn(new PageResponse<>(List.of(), 0, 20, 0, 0));
        mockMvc.perform(get("/api/v1/leads"))
