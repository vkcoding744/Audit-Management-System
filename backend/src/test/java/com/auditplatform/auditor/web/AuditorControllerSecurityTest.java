...
@WebMvcTest(
        controllers = AuditorController.class,
...
class AuditorControllerSecurityTest {
    ...
    @MockBean
    private AuditorEligibilityService eligibilityService;

    @Test
    void listAuditorsUnauthorizedWithoutToken() throws Exception {
        mockMvc.perform(get("/api/v1/auditors")).andExpect(status().isUnauthorized());
    }

    @Test
    @WithMockUser(authorities = "CLIENT_VIEW")
    void listAuditorsForbiddenWithoutPermission() throws Exception {
        mockMvc.perform(get("/api/v1/auditors"))
                .andExpect(status().isForbidden())
                .andExpect(jsonPath("$.error.code").value("SYS_FORBIDDEN"));
    }

    @Test
    @WithMockUser(authorities = "AUDITOR_VIEW")
    void listAuditorsAllowedWithPermission() throws Exception {
        when(auditorService.list(isNull(), isNull(), any())).thenReturn(new PageResponse<>(List.of(), 0, 20, 0, 0));
        mockMvc.perform(get("/api/v1/auditors"))
