...
class AuditControllerSecurityTest {
    ...
    @Test
    void listAuditsUnauthorizedWithoutToken() throws Exception {
        mockMvc.perform(get("/api/v1/audits")).andExpect(status().isUnauthorized());
    }

    @Test
    @WithMockUser(authorities = "CLIENT_VIEW")
    void listAuditsForbiddenWithoutPermission() throws Exception {
    ...
    void listAuditsAllowedWithPermission() throws Exception {
        when(auditService.list(isNull(), isNull(), any())).thenReturn(new PageResponse<>(List.of(), 0, 20, 0, 0));
        mockMvc.perform(get("/api/v1/audits"))
