...
class ExecutionControllerSecurityTest {
    ...
    @Test
    void listResponsesUnauthorizedWithoutToken() throws Exception {
        mockMvc.perform(get("/api/v1/audits/a1/responses")).andExpect(status().isUnauthorized());
    }

    @Test
    @WithMockUser(authorities = "CLIENT_VIEW")
    void listResponsesForbiddenWithoutPermission() throws Exception {
        mockMvc.perform(get("/api/v1/audits/a1/responses"))
    ...
    void listResponsesAllowedWithPermission() throws Exception {
        when(executionService.listResponses("a1")).thenReturn(List.of());
        mockMvc.perform(get("/api/v1/audits/a1/responses"))
