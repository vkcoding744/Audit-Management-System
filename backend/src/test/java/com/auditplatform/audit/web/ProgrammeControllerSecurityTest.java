...
@WebMvcTest(
        controllers = ProgrammeController.class,
...
class ProgrammeControllerSecurityTest {
    ...
    @Test
    void listProgrammesUnauthorizedWithoutToken() throws Exception {
        mockMvc.perform(get("/api/v1/programmes")).andExpect(status().isUnauthorized());
    }

    @Test
    @WithMockUser(authorities = "CLIENT_VIEW")
    void listProgrammesForbiddenWithoutPermission() throws Exception {
    ...
    void listProgrammesAllowedWithPermission() throws Exception {
        when(programmeService.list(isNull(), isNull(), any())).thenReturn(new PageResponse<>(List.of(), 0, 20, 0, 0));
        mockMvc.perform(get("/api/v1/programmes"))
