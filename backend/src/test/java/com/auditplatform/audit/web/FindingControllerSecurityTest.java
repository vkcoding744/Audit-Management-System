...
class FindingControllerSecurityTest {
    ...
    @Test
    void listFindingsUnauthorizedWithoutToken() throws Exception {
        mockMvc.perform(get("/api/v1/findings")).andExpect(status().isUnauthorized());
    }

    @Test
    @WithMockUser(authorities = "CLIENT_VIEW")
    void listFindingsForbiddenWithoutPermission() throws Exception {
    ...
    void listFindingsAllowedWithPermission() throws Exception {
        when(findingService.list(isNull(), isNull(), any())).thenReturn(new PageResponse<>(List.of(), 0, 20, 0, 0));
        mockMvc.perform(get("/api/v1/findings"))
