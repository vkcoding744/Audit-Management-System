...
class DocumentControllerSecurityTest {
    ...
    @Test
    void listDocumentsUnauthorizedWithoutToken() throws Exception {
        mockMvc.perform(get("/api/v1/documents")).andExpect(status().isUnauthorized());
    }

    @Test
    @WithMockUser(authorities = "CLIENT_VIEW")
    void listDocumentsForbiddenWithoutPermission() throws Exception {
    ...
    @Test
    @WithMockUser(authorities = "DOCUMENT_VIEW")
    void listDocumentsAllowedWithPermission() throws Exception {
        when(documentService.list(isNull(), isNull(), isNull(), any())).thenReturn(new PageResponse<>(List.of(), 0, 20, 0, 0));
        mockMvc.perform(get("/api/v1/documents"))
