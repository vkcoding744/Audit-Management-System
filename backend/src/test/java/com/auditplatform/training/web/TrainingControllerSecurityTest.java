...
class TrainingControllerSecurityTest {
    ...
    @Test
    void listTrainingUnauthorizedWithoutToken() throws Exception {
        mockMvc.perform(get("/api/v1/training-records")).andExpect(status().isUnauthorized());
    }

    @Test
    @WithMockUser(authorities = "AUDITOR_VIEW")
    void listTrainingForbiddenWithoutPermission() throws Exception {
    ...
    void listTrainingAllowedWithPermission() throws Exception {
        when(trainingRecordService.list(isNull(), isNull(), any())).thenReturn(new PageResponse<>(List.of(), 0, 20, 0, 0));
        mockMvc.perform(get("/api/v1/training-records"))
