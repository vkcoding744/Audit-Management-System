...
@WebMvcTest(
        controllers = AiGenerationController.class,
...
class AiGenerationControllerSecurityTest {
    ...
    void listUnauthorizedWithoutToken() throws Exception {
        mockMvc.perform(get("/api/v1/ai-generations")).andExpect(status().isUnauthorized());
    }

    @Test
    @WithMockUser(authorities = "CLIENT_VIEW")
    void listForbiddenWithoutPermission() throws Exception {
        mockMvc.perform(get("/api/v1/ai-generations"))
                .andExpect(status().isForbidden())
                .andExpect(jsonPath("$.error.code").value("SYS_FORBIDDEN"));
    }

    @Test
    @WithMockUser(authorities = "AI_VIEW")
    void listAllowedWithPermission() throws Exception {
        when(generationService.list(isNull(), any())).thenReturn(new PageResponse<>(List.of(), 0, 20, 0, 0));
        mockMvc.perform(get("/api/v1/ai-generations"))
