...
Phase 7 (current) adds fieldwork: start a scheduled audit, freeze the checklist, record item results, and complete when required items are assessed. Phases 2–6 remain. Set `AUDIT_PLATFORM_BOOTSTRAP_ADMIN_EMAIL` and `AUDIT_PLATFORM_BOOTSTRAP_ADMIN_PASSWORD` before first start to create the platform super admin. Platform admins must choose a tenant (header `X-Tenant-Id`) before creating clients, standards, auditors, or programmes.
