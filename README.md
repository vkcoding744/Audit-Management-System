...
Phase 8 (current) adds findings and CAPA on in-progress or completed audits. Closed findings cannot be edited; major/minor items need completed CAPA to close. Phases 2–7 remain. Set `AUDIT_PLATFORM_BOOTSTRAP_ADMIN_EMAIL` and `AUDIT_PLATFORM_BOOTSTRAP_ADMIN_PASSWORD` before first start to create the platform super admin. Platform admins must choose a tenant (header `X-Tenant-Id`) before creating clients, standards, auditors, or programmes.
